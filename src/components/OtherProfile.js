...
function OtherProfile(props) {
            ...
            firebase.firestore().collection("user").doc(id).onSnapshot((snap) => {
                ...
                follow.map((value) => {
                    ...
                    for (var k = 0; k < following.length; k++) {
                        if (following[k] === value) {
                            alreadyFollowed = true
                        }
                    }
                    for (var j = 0; j < userList.length; j++) {
                        if (userList[j].id === value) {
                            name = userList[j].data().name
                            followinglength = userList[j].data().following
                        ...
                        <List className={classes.root}>
                            <ListItem alignItems="flex-start">
                                ...
                                {alreadyFollowed ? <Button>Following</Button> : <Button variant="contained"
                                    color="secondary"
                                    onClick={() => {
                                        newFollowing.push(value)
                                        firebase.firestore().collection("user").doc(id).update({
                                            "following": newFollowing
                                        })
                                        hisFollowers.push(id)
                                        firebase.firestore().collection("user").doc(value).update({
    ...
    return (<div> <header className='navbar'>
        ...
        <div className='navbar__item'><Link to="/user" style={{ textDecoration: 'none', color: window.location.pathname.split("/")[1] === 'user' || window.location.pathname.split("/")[1] === 'userprofile' ? '#FF1493' : '#D3D3D3' }}>Users</Link></div>
                            ...
                            <Card className={classes.root}>
                                ...
                                <CardContent>
                                    <Typography variant="body2" color="textSecondary" component="p" style={{ marginLeft: '40px', marginRight: '28px', marginTop: '-15px' }}>
                                        {data.description.split(" ").splice(0, 26).join(" ")} <Avatar style={{ marginTop: '-40px', marginLeft: '415px', backgroundColor: '#FFB6C1' }} >
                                            {/* {data.name[0]} */}
                                        </Avatar>
                                    </Typography>
                                </CardContent>
